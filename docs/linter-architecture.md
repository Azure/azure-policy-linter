...
| `Parameter` | A policy parameter's `Name`, `Type`, `AllowedValues?`, `AllowedValuesExpression?`, `DefaultValue?`, `Metadata?` (raw `JToken` for the unmodeled metadata bag), plus `TryAsConcreteType<T>` for unwrapping to a C# type. |
