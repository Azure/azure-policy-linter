...
The policy definition is referencing a [field alias](https://learn.microsoft.com/en-us/azure/governance/policy/concepts/definition-structure-alias) that maps to a property that exists in one or more older API versions but doesn't exist in the latest API version of the targeted resource type.
