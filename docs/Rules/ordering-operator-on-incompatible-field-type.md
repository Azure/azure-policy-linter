...
The rule fires when the field's type in the latest known API version is incompatible, including when the type is incompatible in every known API version. It does not report incompatibilities limited to older API versions.
