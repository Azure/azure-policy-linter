...
An `allOf` or `anyOf` with at least two children contains a child that is itself an `allOf` or `anyOf` of the same type and also has at least two children. Since both operators are associative, the inner quantifier's children can be merged directly into the outer one, reducing unnecessary nesting.
