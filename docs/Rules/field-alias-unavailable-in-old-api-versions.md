...
The finding may show only the newest affected API versions and the number of newer versions where the property is available. Clients can continue using any supported API version, so consult the resource provider's API-version history to assess the full impact.
...
- Consult the resource provider's API-version history to verify when the field is unavailable; the finding might not list every affected version.
