...
This rule is distinct from `optional-field-alias`. A conditional property does not exist at all unless its condition is met (for example, the wrong trigger kind), whereas an optional property always belongs to the resource type but is simply not required in a request. A single field can trigger both rules; addressing the conditional aspect also resolves the "missing property" concern.
