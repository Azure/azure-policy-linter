...
A description is short. Aim for 150-300 characters; treat 400 as a hard ceiling. No line breaks - the CLI prints it as a single line under the title, and IDE integrations show it as a single tooltip line. No embedded URLs either; the linter emits the rule's documentation link separately as `DocumentationUrl` on every finding (surfaced in both the CLI and JSON output), so runners consume that field rather than resolving the identifier into a link themselves.
