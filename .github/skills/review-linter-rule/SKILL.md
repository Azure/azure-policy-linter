...
- **Engine-code modification.** A rule should normally touch only its own `.cs`, its test, and its doc. Files added or modified under `src/PolicyLinter.Core/Expressions/`, `Parsing/`, `Extensions/`, `Metadata/`, or other root engine paths are a red flag - the logic usually belongs in the rule. **Flag as Error** unless the change addresses an explicitly surfaced expression-model gap needed for the rule's required target or diagnostic location. In that case, verify the engine change is narrow, typed, documented, and directly tested.
...
- **Test names follow `RuleTests_<RuleName>_<Case>` for the default rule set; `RuleTests_<RuleSet>_<RuleName>_<Case>` for non-default rule sets.** Flag any method that omits the rule-set segment for non-default rules, or that incorrectly includes it for default rules.
