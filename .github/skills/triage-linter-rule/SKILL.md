...
8. **Identify and decide on coverage tradeoffs**. Identify cases where a simple/naive linter rule covers 90% of the cases, while 100% coverage would require complex and over-the-top logic. Work with the user to clearly identify the tradeoff and include a recommendation in the spec as to what to do.
  - In many cases, it can hint there are missing core linter capabilities.
  - For example, a linter rule can identify whether a policy targets an NSG by naively checking which aliases the rule uses, or by trying to determine whether NSG resources can satisfy the policy rule, which is basically an NP-complete problem. This is a linter, not a SAT solver.
